...
##Field_containing_trial_names=optional field Point_vector
...
for(i in 1:length(Point_vector)){
  ...
  for(p in 1:length(centroids)){
    ...
    data_plots[[p]] <- bbox2SP(y_max, y_min, x_max, x_min, proj4string=crs(Point_vector))
  }
  ...
  rotated_plots <- elide(data_plots_poly, rotate=bearing,
                         center=c(xmin(Point_vector[i,]), ymin(Point_vector[i,])))
  
  # Assign coordinate system
  crs(rotated_plots) <- crs(Point_vector)
